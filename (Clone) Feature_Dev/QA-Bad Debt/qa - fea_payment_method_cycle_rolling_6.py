...
display(df_fs_payment_03.limit(10))

# COMMAND ----------

# DBTITLE 1,categorical features
ls_test_cat_field = ['payment_method_main_type_6cycle', 'payment_auto_flag_6cycle']

# COMMAND ----------

for i in ls_test_cat_field:
    df_result = (df_fs_payment_03
...
# DBTITLE 1,check misc example
display(df_fs_payment_03
        .filter(f.col('payment_method_main_type_6cycle') == f.lit('misc'))
...
display(df_payment_base
        .filter(f.col('fs_acct_id') ==474867853)
        )
# data missing 
...
ls_test_field = ['payment_method_main_cnt_6cycle', 'payment_method_main_pct_6cycle', 'payment_auto_cnt_6cycle', 'payment_auto_pct_6cycle'
                 ]
