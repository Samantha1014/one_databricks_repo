...
df_base_payment_curr = (
    df_stg_payment
    # convert to nzt 
    .withColumn("payment_effective_dttm_nzt"
    ...
    .filter(
        (f.col('payment_effective_date') <= vt_param_partition_end_date)
        & (f.col('payment_effective_date') >= vt_param_partition_start_date)
    )
    ...
    .join(
        ...
        # cycle attachment 
    .withColumn('partition_date', f.col('payment_date'))
    .join(
        df_param_partition_curr
...
display(
        df_base_payment_curr
        .filter(f.col('fs_acct_id') == '1000024')
)
...
display(df_output_curr
        .filter(f.col('fs_acct_id')=='1000547')
)
...
export_data(
   df=df_output_curr
    , export_path = os.path.join(dir_data_prm, 'prm_coll_action_payment_12m')
    , export_format = 'delta'
