...
df_consec_latepay = spark.read.format('delta').load(os.path.join(dir_dev_parent, 'fea_payment_behavior'))
df_prodcut_acq = spark.read.format('delta').load(os.path.join(dir_dev_parent, 'fea_product_acq'))
...
# MAGIC %md
# MAGIC #### selection on payment and bill features
...
display(df_bill_6cycle
         ...
         .groupBy('reporting_date','charge_delta_max_cat', 'bill_payment_timeliness_status' )
         .agg(f.count('*')
...
df_bill_6cycle =(df_bill_6cycle
         .withColumn('charget_delta_max_cat', f.when(f.col('bill_charge_amt_delta_max_6bmnth') <=1.5, f.lit('L'))
                                               .when( (f.col('bill_charge_amt_delta_max_6bmnth') > 1.5)
...
df_l2_segment_part1= (df_output
        ...
        .withColumn('L2_Segment_part1', f.when( (f.col('cnt_product_add_in_6mp') >=1 ) 
                                                  & (f.col('cnt_late_pay_6bcycle') > 
                                                          f.col('cnt_late_pay_6bcycle_6mp'))
                                              , 'OverCommit'
                                               )
                                         .when( (f.col('cnt_product_add_in_6mp') <1) 
                                                  & (f.col('cnt_late_pay_6bcycle') > 
                                                          f.col('cnt_late_pay_6bcycle_6mp'))
                                             , 'RecentDeterioate')
...
display(df_output
        ...
        #                                       , 'over_commit'
        #                                        )
        #                                  .when(f.col('cnt_late_pay_6bcycle ') > f.col('cnt_late_pay_6bcycle_6mp'), 'recent_detoriate')
        #                                  .otherwise('other')
        # )
        .groupBy('cnt_late_pay_6bcycle_6mp', 'cnt_product_add_in_6mp', 'reporting_date', 'cnt_late_pay_6bcycle')
        .agg(f.count('*').alias('count')
             , f.countDistinct('fs_acct_id')
             , f.countDistinct('fs_srvc_id')
             , f.sum('actual_aod').alias('actual_aod')
...
display(df_output
        ...
        .groupBy('payment_behavior_label', 'reporting_date', 'cnt_late_pay_6bcycle_6mp')
        .agg(f.count('*')
             , f.countDistinct('fs_acct_id')
             , f.sum('actual_aod').alias('actual_aod')
             , f.sum('actual_wo').alias('actual_wo')
...
df_l2_segment_part2 = (df_output
        ...
        .withColumn('L2_segment_part2'
                , f.when( (f.col('pct_pay_after_coll_12m') >0.85)
                  ...
                  .when( (f.col('CNT_LATE_PAY_6BCYCLE') == 0.5 * vt_lookback_billing_cycle)
                         & (f.col('cnt_late_pay_6bcycle_6mp') == 0.5 * vt_lookback_billing_cycle )
                          & (f.col('avg_late_sequence_length') >= 0.5 * 0.5* vt_lookback_billing_cycle ) # 1.5
                         , 'Chronic Late') 
                  .when( (f.col('CNT_LATE_PAY_6BCYCLE') == 0.5 * vt_lookback_billing_cycle) 
...
#dbutils.fs.rm('/mnt/ml-lab/dev_users/dev_sc/99_misc/cohort_seg/df_combine_all_v3', True)

# COMMAND ----------

export_data(
            df = df_combine_all
            , export_path = '/mnt/ml-lab/dev_users/dev_sc/99_misc/cohort_seg/df_combine_all_v3'
             # added aod all for next month 
            , export_format = "delta"
            , export_mode = "append"
...
(
    ...
    .format("snowflake")
    .options(**options)
    .option("dbtable", "lab_ml_store.sandbox.sc_one_off_cohort_seg_combine_all_v3")
    .mode("overwrite")
    .save()
)
...
df_test = spark.read.format('delta').load('/mnt/ml-lab/dev_users/dev_sc/99_misc/cohort_seg/df_combine_all_v3')
