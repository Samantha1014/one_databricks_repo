...
df_coll_action_curr = (
    ...
    .crossJoin(df_reporting_dates)
    .filter(f.col('complete_date') <= f.col('reporting_date'))
    .filter(f.col('complete_date') >= f.add_months('reporting_date', -1))
    # look back 1 months for collection action 
    #.filter(f.col('complete_date') >= f.date_add('reporting_date', -7))
...
display(df_output_curr
        ...
        .filter(
            (f.col('pct_pay_after_coll_12m') > 0.85) # within 5 days in 12 month 
              &(f.col('avg_days_to_payment_after_coll') <=1) # within 1 days in 1 month 
        )
...
# delta_run_node_meta = DeltaTable.forPath(spark,  '/mnt/ml-lab/dev_users/dev_sc/99_misc/cohort_seg/fea_coll_action')
# delta_run_node_meta.delete(
#      (f.col("reporting_date") == "2024-11-03")
# )
...
display(df_test
        .withColumn('pct_pay_after_coll_12m', f.col('cnt_pay_after_coll_12m')/f.col('cnt_in_coll_action_12m'))
        .filter(
            (f.col('pct_pay_after_coll_12m') > 0.85)
