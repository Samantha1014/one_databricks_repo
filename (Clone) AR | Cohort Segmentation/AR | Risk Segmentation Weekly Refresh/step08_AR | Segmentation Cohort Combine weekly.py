...
df_wo_score = spark.read.format('delta').load(os.path.join(dir_mls_parent, 'reporting_cycle_type=rolling cycle'))
...
display(df_coll_action
        .agg(f.max('reporting_date')
             , f.min('reporting_date')
             )
        )

display(df_consec_latepay
        .agg(f.max('reporting_date')
             , f.min('reporting_date')
             )
        )

display(df_prodcut_acq_12m
        .agg(f.max('reporting_date')
             , f.min('reporting_date')
             )
        )

display(df_late_pay_6mp
        .agg(f.max('reporting_date')
             , f.min('reporting_date')
             )
        )
...
vt_reporting_date = (df_fs_unitbase
...
ls_reporting_date =  [vt_reporting_date]
...
display(df_combine_all
        .groupBy( 'reporting_date' ,'L1_segment', 'L2_combine', 'reporting_cycle_type')
...
# DBTITLE 1,Get Septempter 2024 Segment Only
# df_combine_sep24 = (df_test
...
#     .format("snowflake")
#     .options(**options)
#     .option("dbtable", "PROD_ML_STORE.SERVING.AR_SERVICE_RISK_SEGMENTATION_SEP24")
#     .mode("overwrite")
#     .save()
# )
