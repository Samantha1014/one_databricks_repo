...
dbutils.fs.ls('/mnt/ml-lab/dev_users/dev_sc/99_misc/df_model_test_v2')
...
# hold out set foler 
dir_holdout = '/mnt/ml-lab/dev_users/dev_sc/99_misc/df_model_test_v2'
...
df_model_holdout_a = (
    df_model_holdout
    .filter(f.col('reporting_date').isin('2024-01-31', '2024-02-29', '2024-03-31'))
)
...
h2odf_model_holdout = h2o.H2OFrame(df_model_holdout_b.withColumn("reporting_date", f.date_format(f.col("reporting_date"), "yyyy|MM|dd")).toPandas())
...
export_data(
            df = df_output_curr
            , export_path = '/mnt/ml-lab/dev_users/dev_sc/99_misc/predict_aod30d_propensity_v2'
            , export_format = "delta"
            , export_mode = "append"
...
df_test = spark.read.format('delta').load('/mnt/ml-lab/dev_users/dev_sc/99_misc/predict_aod30d_propensity_v2')
...
(
    ...
    .format("snowflake")
    .options(**options)
    .option("dbtable", "lab_ml_store.sandbox.sc_one_off_aod30d_propensity_score_v2")
    .mode("overwrite")
    .save()
)
