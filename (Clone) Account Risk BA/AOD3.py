...
df_bill_base = (df_bill_base
        .withColumn('latest', 
                    f.row_number()
                    .over(Window.partitionBy('account_no','bill_no')
                          .orderBy(f.desc('mod_t'))))
...
# look back at last 6 bills 
df_bill_6mo = (df_bill_base
...
df_receive_6mo =(
    df_payment_6mo.alias('p')
    .join(df_bill_credit_6mo.alias('c'), col('p.account_no') == col('c.account_no'), 'left')
...
display(df_payment_6mo.filter(col('account_no') =='349688317'))

# COMMAND ----------

display(df_payment_6mo.filter(col('account_no') =='337838335'))
...
df_bills_payments = (
    df_bill_6mo.alias('a')
    .join(df_receive_6mo.alias('b'), col('a.account_no') == col('b.account_no'), 'left')
...
# DBTITLE 1,Check Raw Item T
dir_item_t = '/mnt/prod_brm/raw/cdc/RAW_PINPAP_ITEM_T'
df_item_t = spark.read.format('delta').load(dir_item_t)

# COMMAND ----------

dir_item_t_par = 'dbfs:/mnt/ml-lab/dev_users/dev_sc/RAW_PINPAP_ITEM_T' # item_t after partition 
dir_item_t = '/mnt/prod_brm/raw/cdc/RAW_PINPAP_ITEM_T' # raw pinpap item t 

# COMMAND ----------

df_item_t_load = spark.read.format('delta').load(dir_item_t)
df_item_t_par = spark.read.format('delta').load(dir_item_t_par)

# COMMAND ----------

display(df_item_t_par
        .groupBy('poid_type', "CREATED_MONTH" )
...
display( 
        ...
        .select('poid_id0', 'poid_type', 'CREATED_MONTH')
...
# result is the same so means adjustment does capture in both partition table and the source cdc table, just the item_t itself missing adjustment value 

# COMMAND ----------

# DBTITLE 1,Check Bill_T Adjustment Value
df_bill_adjust = (df_bill_base
       .groupBy('account_no')
       .agg(f.sum('adjusted'))
       )
...
df_item_adjust = (df_payment_base
                  .filter(col('poid_type') == '/item/adjustment')
                  .select('account_no', 'created_t', 'poid_id0', 'item_total', 'item_no')
                  .distinct()
                  .groupBy('account_no')
                  .agg(f.sum('item_total'))
                  )
...
# DBTITLE 1,compare
display(
    df_bill_adjust.alias('a')
    .join(df_item_adjust.alias('b'), col('a.account_no') == col('b.account_no'), 'full')
    .withColumn('check', f.when(col('a.account_no').isNotNull() & col('b.account_no').isNotNull(), 'commom')
                .when(col('a.account_no').isNull() & col('b.account_no').isNotNull(), 'in item not in bill')
                .when(col('a.account_no').isNotNull() & col('b.account_no').isNull(), 'in bill not in item')
                .otherwise(0))
    .withColumn('adj_check', abs(f.when(col('sum(item_total)') - col('sum(adjusted)'))>2,1).otherwise(0))
    .groupBy('check')
    .agg(f.count('a.account_no'), f.sum('adj_check'))
)
...
display(
    df_bill_adjust.alias('a')
    .join(df_item_adjust.alias('b'), col('a.account_no') == col('b.account_no'), 'inner')
    .withColumn('adj_check', f.when(col('sum(item_total)') == col('sum(adjusted)'),1).otherwise(0))
    .filter(col('adj_check') ==0)
    .limit(100)
)
...
display(df_payment_base
        .filter(col('poid_type') =='/item/adjustment')
        .select('item_total', 'created_t','item_no', 'poid_id0')
        .distinct()
        .filter(col('account_no') =='334758800'))
        # sum of amount = item_total in item_t adjustment 
