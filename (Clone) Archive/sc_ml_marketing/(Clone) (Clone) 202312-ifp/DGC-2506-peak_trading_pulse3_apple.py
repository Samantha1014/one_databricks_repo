...
df_tmp_excl_07 = (
    ...
    .filter(f.col("reporting_date") == vt_param_reporting_date)
    .filter(f.col("reporting_cycle_type") == vt_param_reporting_cycle_type)
    .filter(f.col("network_dvc_model_brand_std") != 'Apple')
    .select("fs_srvc_id")
...
df_base_score_dr_apple = (
    df_mls_score_dr_apple
    .filter(f.col("reporting_date") == vt_param_reporting_date)
    .filter(f.col("reporting_cycle_type") == vt_param_reporting_cycle_type)
    .select(
        "fs_cust_id"
        , "fs_acct_id"
        , "fs_srvc_id"
        , f.col("propensity_score").alias("dr_apple_score")
        , f.col("propensity_segment_qt").alias("dr_apple_segment")
        , f.col("propensity_top_ntile").alias("dr_apple_top_ntile")
    )
)
...
display(
    df_base_full
    .filter(f.lower(f.col("network_dvc_model_marketing")).rlike("iphone 14|iphone 15"))
...
df_campaign_full = (
    ...
    .withColumn(
        ...
        , f.when(
            (f.col("campaign_segment_lvl1") == 'Apple')
            & (f.lower(f.col("network_dvc_model_marketing")).rlike("iphone 14|iphone 15"))
            , f.lit("N")
        )
        .when(
            (f.col("campaign_segment_lvl1") == 'Apple')
...
(
    ...
    .save('/mnt/ml-lab/dev_users/dev_el/ml_campaigns/ifp/202312_dgc_2506_apple') 
)

# COMMAND ----------

df_campaign_full = spark.read.format("delta").load("/mnt/ml-lab/dev_users/dev_el/ml_campaigns/ifp/202312_dgc_2506_apple")
...
# MAGIC %md ### s206 local control - Apple H
...
df_campaign_cand_target = (
    df_campaign_full
    .filter(f.col("target_segment") == 'a.target')
    .filter(f.col("campaign_cohort") == 'Apple-Y-H')
)
...
# MAGIC %md ### s209 local control - Apple M
...
df_campaign_cand_target = (
    df_campaign_full
    .filter(f.col("target_segment") == 'a.target')
    .filter(f.col("campaign_cohort") == 'Apple-Y-M')
)
...
# MAGIC %md ### s209 local control - Apple L
...
df_campaign_cand_target = (
    df_campaign_full
    .filter(f.col("target_segment") == 'a.target')
    .filter(f.col("campaign_cohort") == 'Apple-Y-L')
    .filter(f.col("dr_apple_segment").isin(['H', 'M']))
    .withColumn(
        "rank"
        , f.row_number().over(
            Window
            .partitionBy(f.lit(1))
            .orderBy(f.desc('dr_apple_score'), f.desc("ifp_score"))
        )
    )
...
def get_campaign_attr(
    ...
    df_out = (
        ...
        .select(
            ...
            , f.lit("2023-11-13").alias("model_base_date")
...
vt_param_campaign_cohort = 'Apple-HP'
vt_param_campaign_name = 'DGC-2506'
...
vt_param_campaign_cohort = 'Apple-MP'
vt_param_campaign_name = 'DGC-2506'
...
vt_param_campaign_cohort = 'Apple-LP'
vt_param_campaign_name = 'DGC-2506'
...
(
    ...
    .format("snowflake")
    .options(**options)
    .option("dbtable", "ml_campaign_ifp_exp_202312_2506")
    .mode("overwrite")
    .save()
)
...
SELECT 
	...
	, concat(TAB_A.CAMPAIGN_NAME, '-DR-', TAB_A.CAMPAIGN_SEGMENT_LVL1, '-', TAB_A.CAMPAIGN_SEGMENT_LVL4) AS VARIABLE_23
FROM STAGING.DBO.ML_IFP_EXP_202312_2506 TAB_A
---where tab_a.PRIM_ACCS_NUM = '64210471477'
...
SELECT 
   CAR_DATE
    ...
    , CAST('2023-11-16' as date) AS CAMPAIGN_DATE
    , CAMPAIGN_NAME
    , COHORT
    , GETDATE() AS DATA_CREATE_DATE
FROM STAGING.DBO.ML_IFP_EXP_202311_2494;
'''
