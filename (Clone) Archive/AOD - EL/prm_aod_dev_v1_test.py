...
display(df_stg_payment
        .filter(f.col('item_poid_type')
                =='/item/payment'
                )
        .select('item_poid_id0', 'fs_acct_id')
        .agg(
...
display(df_base_bill_init_curr
        .agg(f.count('bill_no')
...
display(df_base_bill_init_curr
        ...
        .filter(f.col('fs_acct_id') == 371055557)
       )
...
df_base_bill_calc_start_curr = (
    df_base_bill_init_curr
    .filter(f.col("due") == 0) 
    .filter(f.col("bill_index") >= 6) # close from 6 cycle before  
    .withColumn(
        "temp_index"
        , f.row_number().over(
            Window
...
display(df_base_bill_calc_start_curr
        .filter(f.col('fs_acct_id') =='490821909')
        )
...
display(df_base_bill_calc_start_curr
        .filter(f.col('bill_index_start') ==43)
        #.groupBy('bill_index_start')
...
display(
    df_base_payment_init_curr
       .filter(f.col('fs_acct_id')  ==1005779)
       )
...
display(df_base_payment_aggr_curr
        .filter(f.col('fs_acct_id') == '337838335')
        )
