...
# DBTITLE 1,late pay bills proportion
display(
    df_prm_bill
    .filter(f.col("total_due") > 0)
    .select("bill_no", "fs_acct_id", "bill_due_date", "bill_close_date")
...
# MAGIC %md
# MAGIC ### S05 IFP accessory 

# COMMAND ----------

display(df_fea_ifp_accessory
        .filter(f.col('ifp_acct_accs_flag') == 'Y')
        .limit(1000))

# COMMAND ----------

# DBTITLE 1,ifp acc base distribution
display(df_fea_ifp_accessory
        .select('fs_acct_id', 'ifp_acct_accs_flag', 'reporting_date', 'reporting_cycle_type')
        .distinct()
        .groupBy('reporting_date', 'ifp_acct_accs_flag')
        .agg(f.countDistinct('fs_acct_id')
             , f.count('*')
             )
        )
...
# DBTITLE 1,ifp accessory on wo base
display(
    df_wo_base
    ...
    .groupBy(f.date_format('rec_created_dttm', 'yyyy-MM').alias('wo_month')
             , 'acc_flag'
             )
    .agg(f.countDistinct('fs_acct_id')
         , f.sum('write_off_item_amount')
...
# DBTITLE 1,ifp device with assory
display(
    df_ifp_base
    .distinct()
    .join(df_fea_ifp_accessory
        .select('fs_acct_id', 'ifp_acct_accs_flag', 'ifp_acct_accs_cnt'
                , 'ifp_acct_accs_term_end_date_max', 'reporting_date'
                )
        .distinct()
        .filter(f.col('ifp_acct_accs_flag') == f.lit('Y'))
        , ['fs_acct_id', 'reporting_date'], 'left'
         )
    .filter( (f.col('ifp_prm_dvc_term_end_date') >= f.col('ifp_acct_accs_term_end_date_max')) | 
       (f.col('ifp_acct_accs_term_end_date_max').isNull()))
    .withColumn('acc_flag', 
                f.when(f.col('ifp_acct_accs_flag').isNull(), 'no accesory'
                )
    ...
    .groupBy(f.date_format('reporting_date', 'yyyy-MM').alias('rpt_month')
             , 'acc_flag'
             )
    .agg(f.countDistinct('fs_acct_id')
         #, f.sum('write_off_item_amount')
...
display(df_fea_ifp_accessory.limit(10))
...
display(df_wo_ifp_final
        .groupBy('wo_month', 'acces_in_wo','ifp_cat')
        .agg(f.countDistinct('fs_acct_id')
