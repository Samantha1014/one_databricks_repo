...
# MAGIC %md
# MAGIC iPhone SE launch 
# MAGIC 1. customers who are using previous SE phone models
# MAGIC 2. who are using phone 11 or older model 
...
# DBTITLE 1,check ml store propensity
display(
    df_mls_score_dr
    .filter(f.col('reporting_date') == vt_param_reporting_date)
    #.filter(f.col('propensity_top_ntile') >=91)
    .filter(f.col('propensity_segment_qt') == 'H')    
)

# COMMAND ----------

# DBTITLE 1,check ifp srvc
display(df_fs_ifp_srvc
...
# DBTITLE 1,check ifp bill
display(
    df_fs_ifp_bill
    .select('fs_cust_id', 'fs_acct_id', 'ifp_bill_dvc_term_end_date', 'ifp_bill_dvc_term_start_date'
        , 'fs_ifp_bill_dvc_id', 'fs_ifp_bill_dvc_order_id', 'ifp_bill_dvc_model', 'ifp_bill_dvc_model_brand'
        )
    .filter(f.col('fs_ifp_bill_dvc_id').isNotNull())
    .limit(100)
    .distinct()
    #.count()
)
...
df_base_score_dr_apple = (
    ...
    .select(
        ...
        , "fs_acct_id"
        , "fs_srvc_id"
        , f.col("propensity_score").alias("dra_score")
        , f.col("propensity_segment_qt").alias("dra_segment")
        , f.col("propensity_top_ntile").alias("dra_top_ntile")
    )
)
...
df_proc_full = (
    ...
    .withColumn("dra_top_nitle", f.ntile(30).over(Window.orderBy(f.desc("dra_score"))))
)
