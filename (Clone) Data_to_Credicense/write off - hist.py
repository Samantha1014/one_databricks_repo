...
vt_one_off_look_back_period = '2019-04-01'

# COMMAND ----------

# DBTITLE 1,history version
...
df_wo_base_hist = (
    ...
    .filter( f.col('create_date') >= vt_one_off_look_back_period)
              )


# COMMAND ----------

display(df_wo_base_hist)

# COMMAND ----------

## for history bulk load, if there is multiple billing account in one customer, we only pick the latest write off date per customer 
df_wo_hist = (
              df_wo_base_hist.alias('a')
                .join(df_sbl_output, ['account_no'], 'left')
                .withColumn('rnk', f.row_number().over(
                                      Window.partitionBy('customer_id')
                                      .orderBy(f.desc('create_date'))
                                      )
                            )
                .filter(f.col('rnk') ==1)  # pick the latest records 
                .select(
                      ##'account_no'
                       'customer_id'
...
display(df_output_01.limit(100))
