...
df_months = spark.range(-6,0)

df_lookback_months = (
    df_months
    .withColumn('current_date', f.current_date())
    .withColumn("reporting_date",
     f.last_day(f.add_months('current_date', 'id')))
)
    

# COMMAND ----------

#display(df_lookback_months)
...
df_aod_output = (df_aod
        ...
        .crossJoin(f.broadcast(df_lookback_months))
...
df_output = (df_aod_output
             ...
             .withColumn('account_no', f.lpad(f.col("account_no").cast("string"), 9, '0')) # get leading zero 
             )
